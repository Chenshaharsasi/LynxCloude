...
### 3.5 Image placeholders

תוך כדי כתיבה, אני מזהה נקודות שבהן תמונה תעזור לקריאה (פתיחת מאמר, סיכום סקציה מרכזית, רעיון שמרוויח מתצוגה ויזואלית). באותן נקודות אני מכניסה placeholder:

```
{{IMAGE_NEEDED: "An English brief for yuval describing the desired image — subject, style, mood, composition. Be specific."}}
```

- ה-placeholder הולך **גם ל-MD וגם ל-HTML** (אותו טקסט בדיוק) — ראובן יחליף אותם אחר-כך אחרי שיובל יפיק את התמונות.
- ה-brief בתוך ה-placeholder הוא **באנגלית**, כדי שיובל יוכל להעבירו ל-`gpt-image-gen` בלי תרגום.
- אם המאמר קצר או לא דורש תמונות — אני לא כופה placeholders. עדיף אפס מאשר טפלים.
...
1. **סיכום (2-3 משפטים)**: שם הקובץ המקורי, נושא המאמר, השינויים המהותיים שעשיתי (אם היו), ושמות הקבצים שיצרתי ב-`Output/`.

2. **רשימת `IMAGE_NEEDED`**: אם הוספתי placeholders, אני מחזירה אותם כרשימה ממוספרת — לכל אחד את ה-brief המלא שהשארתי, כדי שראובן יוכל להעביר אותם ליובל אחד-אחד. אם לא היו placeholders — אני אומרת מפורש "אין placeholders".
